...
def find_repo_root(start):
    for path in [start, *start.parents]:
        if (path / "results").exists() and (path / "visualizations").exists():
            return path
...
OUTPUT_DIR = REPO_ROOT / "visualizations" / "appendix" / "image" / "appendix_fourier_projections"
POSITION = "-1"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot image Fourier projection appendix panels.")
...
def load_projection(path):
    ...
    residues = torch.tensor([label_value(row, target) % period for row in data["labels"]])
...
def save_layer_comparisons():
    ...
    for modality in ["addition", "subtraction"]:
        ...
        if len(items) != len(layers):
            continue
        stem = f"projection_image_{modality}_{target}_T{period}_layers34_35_38"
        outputs += save_projection_grid(
            ...
            legend_title=f"{label_modality(modality)} mod {period}",
        )
    return outputs
...
def save_layer44_period_comparisons():
    ...
    for modality in ["addition", "subtraction"]:
        ...
        if len(items) != len(periods):
            continue
        stem = f"projection_image_{modality}_{target}_layer{layer}_periods2_5_10_20_50_100"
        outputs += save_projection_grid(
            items=items,
